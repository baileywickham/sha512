...
func pad(message []byte) []byte {
	...
	// Append a 1, then k -1 bits of 0s.
	zero_pad := ((k - 1) / 8)
...
func hash(Message [][]uint64, H []uint64) []uint64 {
	for _, value := range Message {
		H = hashMessage(value, H)
	}
	return H
}

func hashMessage(messageBlock []uint64, H []uint64) []uint64 {
	W := make([]uint64, 80)
	for t := 0; t <= 15; t++ {
		W[t] = messageBlock[t]
	}
